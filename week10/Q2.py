...
def get_failed_attempts(username):
    ...
    cursor = conn.cursor()
    cursor.execute
    ("SELECT * FROM login_attempt WHERE username = ? AND success = 0",
    (username)
    )
...
def count_failures_per_user():
    ...
    cursor = conn.cursor()
    cursor.execute
    ("SELECT username, COUNT(*) FROM login_attempt WHERE success = 0 GROUP BY username"
    )
    rows = cursor.fetchall()
    conn.close()
    return rows
...
def delete_old_attempts(username):
    ...
    cursor = conn.cursor()
    cursor.execute
    ("DELETE FROM login_attempt WHERE username = ?", (username,)
    )
